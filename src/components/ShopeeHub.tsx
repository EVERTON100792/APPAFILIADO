...
export const ShopeeHub: React.FC<ShopeeHubProps> = ({ onShowToast, userStoreSlug: propStoreSlug, onViralize, onSaveHistory }) => {
  ...
  const handleSearch = async (overrideKeyword?: string, forceRefresh = false) => {
    ...
    const off50Keys = ["50% off", "liquidação", "desconto"];
    ...
    // Variar página de forma segura (Abas especiais tem menos páginas de sucesso)
    let maxPage = (activeTab === "all") ? 8 : 3;
    let randomPage = Math.floor(Math.random() * maxPage) + 1;

    try {
      // Estágio 1: Busca Inicial
      ...
      // Se veio muito pouco, buscar mais uma página para garantir volume
      ...
      // Fallback Estágio 2: Relevância
      if (finalProducts.length === 0) {
        console.warn(`Fallback Estágio 2: Relevância para "${searchKeyword}"`);
      ...
      onShowToast("?? Erro na conexão Shopee");
    } finally {
      setIsSearching(false);
    }
  };
  ...
  const handleImageSelection = async (product: any) => {
    try {
      ...
      // Se ainda não tiver imagens, usar a principal do produto como fallback
  ...
  const addToVitrine = async (product: ShopeeProduct) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) { onShowToast("?? FAÇA LOGIN!"); return; }
            ...
            <div className="fixed inset-0 z-[3001] flex flex-col bg-slate-950 overflow-hidden">
              ...
              >
                ...
                <div className="p-6 bg-slate-900 border-t border-white/10 flex flex-col gap-3">
                  ...
                  <p className="text-[8px] font-bold text-center text-white/20 uppercase">Selecione as fotos da Shopee ou suba as suas para o vídeo</p>
                </div>
              </motion.div>
            </div>
          )}
